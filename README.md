# Мобильная разработка

## Содержание

1. [Лидерборд](./leaderboard.ods)
1. [Roadmap](#roadmap)
1. [Как выполнять задания](#как-выполнять-задания)
1. [Документация](#документация)
1. [Полезное](#полезное)

## Roadmap

Все, что находится выше `Мы здесь` должно быть у вас для успешного получения зачета

### 1. Основы веба

#### 1.1. Основы HTML/JS

[HTMLAcademy](https://htmlacademy.ru/courses/297) 

#### 1.2. Основы HTML/CSS

[HTMLAcademy](https://htmlacademy.ru/courses/299) _<-- Мы здесь_

#### 1.3. Расположение элементов (CSS)

-   [Flexbox Frog](https://flexboxfroggy.com/#ru)
-   [Grid Garden](https://cssgridgarden.com/#ru)

_Опционально_
-   [CSS Diner](https://flukeout.github.io/)
-   [Flexbox Defence](http://www.flexboxdefense.com/)
-   [Flex Zombies](https://mastery.games/flexboxzombies/)

#### 1.4. JavaScript

-   [CodeWars](https://www.codewars.com/)

#### 1.5. Git

-   [Ohmygit!](https://ohmygit.org/)

#### 1.6. [Задание 1](https://github.com/31isp/mobdev3-lab1)

## Софт

- [NodeJS](https://nodejs.org/en)

_Дома лучше использовать [https://github.com/Schniz/fnm](fnm)_

-   [Git](https://git-scm.com/downloads)

-   Любой редактор кода (желательно [Visual Studio Code](https://code.visualstudio.com/)

## Документация

-   [JavaScript](https://developer.mozilla.org/ru/docs/Web/JavaScript)
-   [CSS](https://developer.mozilla.org/ru/docs/Web/CSS)
-   [HTML](https://developer.mozilla.org/ru/docs/Web/HTML)
-   [React](https://ru.react.dev/)
-   [React Native](https://reactnative.dev/)

## Полезное

1. [Изучите X за Y минут (HTML)](https://learnxinyminutes.com/docs/ru-ru/html-ru/)
2. [Изучите X за Y минут (JavaScript)](https://learnxinyminutes.com/docs/ru-ru/javascript-ru/)
3. [Изучите X за Y минут (TypeScript)](https://learnxinyminutes.com/docs/ru-ru/typescript-ru/)
4. [Learn X in Y (Git)](https://learnxinyminutes.com/docs/git/)
